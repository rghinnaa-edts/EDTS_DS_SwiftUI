...
struct ButtonStepperView: View {
    ...
    /// True when showing just the quantity number (not active, qty > 0) —
    /// this state gets its own distinct white/blue30 styling.
    private var isShowingCount: Bool {
        !isActive && quantity > 0
    }
    ...
    var body: some View {
        ZStack(alignment: .trailing) {
            ...
            RoundedRectangle(cornerRadius: circleSize / 2)
                .fill(isShowingCount ? EDTSColor.white : (variant == .blue ? accentColor : EDTSColor.white))
                .overlay(
                    Group {
                        if isShowingCount {
                            RoundedRectangle(cornerRadius: circleSize / 2)
                                .stroke(EDTSColor.blue50, lineWidth: 1)
